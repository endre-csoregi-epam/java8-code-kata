...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void howManyItemsWanted() {
         ...
         */
//        long sum = customerList.stream().map(Customer::getWantToBuy).flatMap(Collection::stream).count();
        long sum = customerList.stream().mapToInt(customer -> customer.getWantToBuy().size()).sum();
